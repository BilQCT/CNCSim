...
np.save(f"./figures/cnc_measurement_adjusted_n_{n_min}_{n_max}_{n_delta}_m_{m}_beta_{beta_min}_{beta_max}_{delta}.npy", np.array(results))
...
plt.title(f"CNC: Measurement Time vs. n for Different $\\beta$ Values (m={m})")
plt.legend()
plt.grid(True)
